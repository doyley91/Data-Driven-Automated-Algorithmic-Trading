...
def main(tickers=['AAPL'], start=None, end=None, n_steps=21):
    ...
    for ticker in tickers:
        ...
        ax.set(title='{} Boosted Trees In-Sample Prediction'.format(ticker), xlabel='time', ylabel='$')
        ax.legend(['Original $', 'Prediction $'])
        fig.tight_layout()
        fig.savefig('charts/{}-Boosted-Trees-In-Sample-Prediction.png'.format(ticker))

        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot(forecast_data[ticker]['adj_close'][-n_steps:])
        ax.set(title='{} Day {} Boosted Trees Out-of-Sample Forecast'.format(n_steps, ticker), xlabel='time',
               ylabel='$')
        ax.legend(['Forecast $'])
        fig.tight_layout()
        fig.savefig('charts/{}-Day-{}-Boosted-Trees-Out-of-Sample-Forecast.png'.format(n_steps, ticker))
